...
import torch
import os
from AGNeRF.mlp_network import IBRNet
from AGNeRF.feature_network import ResUNet
...
class AGNeRFModel(object):
    def __init__(self, args, load_opt=True, load_scheduler=True):
        self.args = args
        device = torch.device('cuda:{}'.format(args.local_rank))
        # create coarse IBRNet
        self.net_coarse = IBRNet(args,
                                 net_depth=self.args.netdepth, 
                                 net_width=self.args.netwidth,
                                 input_ch_feat=self.args.coarse_feat_dim).to(device)
        # self.net_coarse = IBRNet(args,
        ...
        if args.coarse_only:
            ...
            self.net_fine = IBRNet(args,
                                   net_depth=self.args.netdepth_fine, 
                                   net_width=self.args.netwidth_fine,
                                   input_ch_feat=self.args.fine_feat_dim).to(device)
            # self.net_fine = IBRNet(args,
