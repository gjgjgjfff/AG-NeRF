...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1):
        ...
        self.scene_scaling_factor = data['scene_scaling_factor']
        self.scene_origin = data['scene_origin']
    ...
    def get_rays_single_image(self, H, W, intrinsics, c2w):
        ...
        # rays_d = torch.sum(dirs[..., np.newaxis, :] * c2w[:3,:3], -1).reshape(-1, 3)
        # rays_o = c2w[:3,-1].expand(rays_d.shape).reshape(-1, 3)
    ...
    def get_all(self):
        ret = {'ray_o': self.rays_o.cuda(),
               ...
               'src_cameras': self.src_cameras.cuda() if self.src_cameras is not None else None,
                'scene_scaling_factor': self.scene_scaling_factor.cuda() if self.scene_scaling_factor is not None else None,
               'scene_origin': self.scene_origin.cuda() if self.scene_origin is not None else None
        }
        return ret
    ...
    def sample_random_pixel(self, N_rand, sample_mode, center_ratio=0.8):
        if sample_mode == 'center':
            ...
            # 这里调用了随机数生成器rng中的choice函数。choice函数用于从给定的序列中随机抽取项目
    ...
    def random_sample(self, N_rand, sample_mode, center_ratio=0.8):
        ...
        select_inds = self.sample_random_pixel(N_rand, sample_mode, center_ratio) # 随机采样N_rand个像素
