...
from AGNeRF.sample_ray import RaySamplerSingleImage
from AGNeRF.render_image import render_single_image
from AGNeRF.model import AGNeRFModel
from utils import *
from AGNeRF.projection import Projector
from AGNeRF.data_loaders import dataset_dict
from torch.utils.data import DataLoader
from TimeSpent import TimeSpent

os.environ["CUDA_VISIBLE_DEVICES"]="1"

from evaluation_utils import rgb_evaluation
...
if __name__ == '__main__':
    ...
    # Create IBRNet model
    model = AGNeRFModel(args, load_scheduler=False, load_opt=False)

    eval_dataset_name = args.eval_dataset
    extra_out_dir = 'eval/{}/{}'.format(eval_dataset_name, args.expname)
    ...
    f = os.path.join(out_scene_dir, 'args.txt')
    ...
    if args.config is not None:
        f = os.path.join(out_scene_dir, 'config.txt')
        if not os.path.isfile(f):
            shutil.copy(args.config, f)


    coarse_out_scene_dir = os.path.join(out_scene_dir, 'coarse')
    fine_out_scene_dir = os.path.join(out_scene_dir, 'fine')
    
    coarse_out_rgb_scene_dir = os.path.join(coarse_out_scene_dir, 'pred_rgb')
    fine_out_rgb_scene_dir = os.path.join(fine_out_scene_dir, 'pred_rgb')
    coarse_out_depth_scene_dir = os.path.join(coarse_out_scene_dir, 'pred_depth')
    fine_out_depth_scene_dir = os.path.join(fine_out_scene_dir, 'pred_depth')
    
    os.makedirs(coarse_out_rgb_scene_dir, exist_ok=True)
    os.makedirs(fine_out_rgb_scene_dir, exist_ok=True)
    
    os.makedirs(coarse_out_depth_scene_dir, exist_ok=True)
    os.makedirs(fine_out_depth_scene_dir, exist_ok=True)
    ...
    for i, data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            coarse_psnr, coarse_ssim, coarse_lpips = rgb_evaluation(gt_rgb_np, coarse_pred_rgb_np)


            # saving outputs ...
            coarse_pred_rgb = (255 * np.clip(coarse_pred_rgb.numpy(), a_min=0, a_max=1.)).astype(np.uint8)
            imageio.imwrite(os.path.join(coarse_out_rgb_scene_dir, '{}_pred_coarse_psnr{}_ssim{}_lpips{}.png'.format(file_id, coarse_psnr, coarse_ssim, coarse_lpips)), coarse_pred_rgb)
            ...
            coarse_disp_visual = visualize_depth(coarse_pred_depth.numpy())
            depth_savedir = os.path.join(coarse_out_depth_scene_dir, '{}_depth_vis_coarse_psnr{}_ssim{}_lpips{}.png'.format(file_id, coarse_psnr, coarse_ssim, coarse_lpips))
            cv2.imwrite(depth_savedir, coarse_disp_visual)
            ...
            # coarse_pred_depth_colored = colorize_np(coarse_out_depth_scene_dir,
            #                                         range=tuple(data['depth_range'].squeeze().cpu().numpy()))
            # imageio.imwrite(os.path.join(out_scene_dir, '{}_depth_vis_coarse_psnr{}_ssim{}_lpips{}.png'.format(file_id, coarse_psnr, coarse_ssim, coarse_lpips)),
            ...
            if ret['outputs_fine'] is not None:
                ...
                fine_psnr, fine_ssim, fine_lpips = rgb_evaluation(gt_rgb_np, fine_pred_rgb_np)
                ...
                imageio.imwrite(os.path.join(fine_out_rgb_scene_dir, '{}_pred_fine_psnr{}_ssim{}_lpips{}.png'.format(file_id, fine_psnr, fine_ssim, fine_lpips)), fine_pred_rgb)




                fine_pred_depth = ret['outputs_fine']['depth'].detach().cpu()
                fine_disp_visual = visualize_depth(fine_pred_depth.numpy())
                depth_savedir = os.path.join(fine_out_depth_scene_dir, '{}_depth_vis_fine_psnr{}_ssim{}_lpips{}.png'.format(file_id, fine_psnr, fine_ssim, fine_lpips))
                cv2.imwrite(depth_savedir, fine_disp_visual)
                ...
                #                                       range=tuple(data['depth_range'].squeeze().cpu().numpy()))
                # imageio.imwrite(os.path.join(fine_out_depth_scene_dir, '{}_depth_vis_fine_psnr{}_ssim{}_lpips{}.png'.format(file_id, fine_psnr, fine_ssim, fine_lpips)),
    ...
    formatted_results = str(results_dict).replace(", ", ",\n")
    formatted_results = formatted_results.replace("{", "{\n") 
    formatted_results = formatted_results.replace("}", "\n}")

    f = open("{}/psnr_{}_{}.txt".format(out_scene_dir, save_prefix, model.start_step), "w")
    f.write(str(formatted_results))
    f.close()
